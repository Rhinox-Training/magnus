...
namespace Rhinox.Magnus.Editor
{
    public class SceneLoaderButton : BaseToolbarIconDropdown<SceneReference>
    {
        ...
        [SerializeField, HideInInspector] private string _iconB64;
        [SerializeField, HideInInspector] private int _iconWidth;
        [SerializeField, HideInInspector] private int _iconHeight;

        protected override Texture Icon 
        {
            get
            {
                if (_icon == null)
                {
                    _icon = new Texture2D(_iconWidth, _iconHeight);
                    byte[] imageBytes = System.Convert.FromBase64String(_iconB64); //base64 string to byte[]
                    _icon.LoadImage(imageBytes);
                }
        ...
        public SceneLoaderButton()
        {
            var tex = UnityIcon.InternalIcon("d_SceneAsset Icon");

            if (tex.isReadable)
            {
                var bytes = tex.EncodeToPNG();
                _iconB64 = Convert.ToBase64String(bytes);
                _iconWidth = tex.width;
                _iconHeight = tex.height;
            }
            else
            {
                DebugUtils.RenderTextureToPng(tex, out _iconB64);
                _iconWidth = tex.width;
                _iconHeight = tex.height;
            }
        }
        ...
        protected override void SelectionMade(IEnumerable<SceneReference> selection)
        {
            var scene = selection.FirstOrDefault();
